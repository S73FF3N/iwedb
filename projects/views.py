...
from .models import Project, Comment, Calculation_Tool, OfferNumber, Reminder, PoolProject, Document, CustomerQuestionnaire, Turbine_CustomerQuestionnaire, questionnaire_translation_dict
...
class CustomerQuestionnaireWizard(SessionWizardView):
    ...
    def get_form_initial(self, step):
        ...
        if step in ["turbineID", "manufacturer", "turbine_model", "hub_height"]:
    ...
    def get_form_kwargs(self, step):
        kwargs = super( CustomerQuestionnaireWizard, self).get_form_kwargs()
    ...
    def done(self, form_list, form_dict, **kwargs):
        ...
        for step_name in form_dict.keys():
            ...
            if not step_name == "contact":
                if not step_name in ["turbineID", "manufacturer", "turbine_model", "hub_height"]:
    ...
    def get_form_initial(self, step):
        if step in ["turbineID", "manufacturer", "turbine_model", "hub_height"]:
            ...
            if data is not None:
                extra = data["amount_wec"]
                form_class.extra = extra
        return super(CustomerQuestionnaireEdit, self).get_form_initial(step)
