...
class WEC_TypFilter(django_filters.FilterSet):
    ...
    manufacturer = django_filters.ModelMultipleChoiceFilter(queryset=Manufacturer.objects.filter(turbine_model_manufacturer=True), widget=autocomplete.ModelSelect2Multiple(url='turbines:manufacturer-autocomplete'))
