...
class EventListFilter(django_filters.FilterSet):
    ...
    responsible = django_filters.ModelMultipleChoiceFilter(queryset=User.objects.filter(groups__name__in=["Technical Operations"]), widget=autocomplete.ModelSelect2Multiple(url='turbines:technical-operations-autocomplete'), label=_('Responsible'))
